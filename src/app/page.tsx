...
export default function Home() {
  ...
  return <div className="app-shell">
    ...
    <div className="layout">
      ...
      <main className="main">
        {toast && <div className="tip" role="status"><strong>Done.</strong> {toast}</div>}
        ...
        {view === "passport" && <Passport onFinding={goToFinding} />}
...
function Copilot({ projectId }: { projectId:string }) {
  ...
  return <><div className="header-row"><div><p className="eyebrow">Cited document intelligence</p><h1>Project copilot</h1><p className="sub">Ask open-ended questions. Answers stay constrained to Orion DC-01 evidence.</p></div><Badge value="EVIDENCE MODE" kind="ready" /></div><div className="card"><div className="chat" aria-live="polite">{messages.length===0 && <div className="chat-answer">Ask a project question, for example: <strong>“Why is UPS-01 blocked?”</strong>, <strong>“Which findings threaten commissioning?”</strong>, or <strong>“What corrective action is recommended for the generator?”</strong></div>}{messages.map((message,index)=><div className="chat-exchange" key={`${message.question}-${index}`}><div className="chat-question">{message.question}</div><div className={`chat-answer ${message.mode === "insufficient" ? "chat-insufficient" : ""}`}><div className="chat-answer-head"><span>{message.mode === "gemini" ? "Grounded answer" : message.mode === "insufficient" ? "Insufficient evidence" : "Safe fallback"}</span>{message.grounded && <Badge value="CITED" kind="ready" />}</div><div>{message.answer}</div>{message.citations.length>0 && <div className="citation-grid">{message.citations.map((citation,citationIndex)=><article className="citation-card" key={`${citation.source}-${citation.page}-${citationIndex}`}><div className="citation-source">{citation.source} · p.{citation.page}</div><p>{citation.excerpt}</p></article>)}</div>}</div></div>)}{loading && <div className="chat-answer chat-loading">Retrieving relevant project evidence and preparing a cited answer…</div>}</div>{error && <div className="chat-error" role="alert">{error}</div>}<form className="chat-form" onSubmit={ask}><input className="input" value={question} maxLength={2000} disabled={loading} onChange={event=>setQuestion(event.target.value)} placeholder="Ask about any requirement, asset, document, or open finding…"/><button className="button primary" type="submit" disabled={loading || !question.trim()}>{loading ? "Checking evidence…" : "Ask copilot"}</button></form></div></>;
}
